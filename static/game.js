...
async function spin() {
  ...
  $('result').textContent = `🌟 ${res.result?.label || 'Win'} → +${res.payout} SharkCoins${label}`;
...
function renderStore(packages = []) {
  ...
  packages.forEach((pack, index) => {
    ...
    card.innerHTML = `
      ...
      <p class="pack-stats">${pack.energy}⚡ energy + ${pack.bonus_spins}🎡 wheel tokens</p>
      <strong>${pack.stars} ⭐</strong>
      <button class="primary" data-pack="${pack.id}">Buy for ${pack.stars}⭐</button>
    `;
    container.appendChild(card);
  });
}
...
function buyStarPack(packId) {
  const botUsername = tg?.initDataUnsafe?.receiver?.username || tg?.initDataUnsafe?.user?.username || 'YOUR_BOT_USERNAME';
  const url = `https://t.me/${botUsername}?start=buy_${packId}`;
